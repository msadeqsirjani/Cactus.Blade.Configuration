...
namespace Cactus.Blade.Configuration
{
    ...
    public sealed class AppSettings
    {
        ...
        /// </exception>
        public string this[string key] => Config.Root[$"AppSettings:{key}"] ?? throw GetKeyNotFoundException(key);

        private static Exception GetKeyNotFoundException(string key)
        {
            return new KeyNotFoundException(
                $"Unable to locate {nameof(Config.AppSettings)} key '{key}' in {typeof(Config).FullName}.{nameof(Config.Root)}.");
        }
    }
}
