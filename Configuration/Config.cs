...
namespace Cactus.Blade.Configuration
{
    ...
    public static class Config
    {
        ...
        private static IConfiguration GetDefaultRoot(IEnumerable<KeyValuePair<string, string>> additionalValues)
        {
            ...
            if (_basePath.IsNotNull())
                builder.SetBasePath(_basePath);

            builder
                .AddAppSettingsJson(true)
                .AddEnvironmentVariables();

            if (additionalValues.IsNotNull())
        ...
        private static IConfigurationBuilder AddSecrets(this IConfigurationBuilder builder)
        {
            try
            {
                const string extensionTypeName = "Secrets.ConfigurationBuilderExtensions, Secrets";
                var extensionType = Type.GetType(extensionTypeName)?.GetTypeInfo();

                if (extensionType.IsNotNull())
                {
                    var addRockLibSecretsMethod = extensionType.GetMethod("AddSecrets",
                        new[] { typeof(IConfigurationBuilder) });

                    if (addRockLibSecretsMethod.IsNotNull())
